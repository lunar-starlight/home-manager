...
before packages are loaded."
  ...
  (add-hook 'whitespace-mode-hook (lambda () (delete 'lines    whitespace-style)))
  (add-hook 'whitespace-mode-hook (lambda () (delete 'trailing whitespace-style)))
  (add-hook 'whitespace-mode-hook (lambda () (delete 'newline  whitespace-style)))
  (add-hook 'whitespace-mode-hook
            (lambda () (setq whitespace-display-mappings
                             '((space-mark   ?\    [?\xB7]       [?.])
                               (space-mark   ?\xA0 [?\xA4]       [?_])
                               (tab-mark     ?\t   [?\x21E5 ?\t] [?\xBB ?\t])))))
  ;;(add-to-list auto-mode-alist '("\\.lagda.md\\'" . agda2-mode))
  (setf powerline-scale 2)
